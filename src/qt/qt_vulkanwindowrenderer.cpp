...
private:
    void updateOptions() {
        ...
        if ((res = m_devFuncs->vkCreateDescriptorPool(m_window->device(), &poolInfo, nullptr, &descPool)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create descriptor pool. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        }
        ...
        if ((res = m_devFuncs->vkAllocateDescriptorSets(m_window->device(), &allocInfo, &descSet)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create descriptor set. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        }}
...
public:
    ...
    void initResources() override
    {
        ...
        if (vmaCreateAllocator(&info, &allocator) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create Vulkan allocator. Switch to another renderer");
            return;
        }
        ...
        if ((res = vmaCreateImage(allocator, &imageInfo, &allocInfo, &image, &allocation, &allocatedInfo)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create Vulkan image. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        };
        ...
        if ((res = m_devFuncs->vkCreateImageView(m_window->device(), &imageViewInfo, nullptr, &imageView)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create Vulkan image view. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        }
        ...
        // Begin Pipeline creation.
        {
            ...
            if ((res = m_devFuncs->vkCreateDescriptorSetLayout(m_window->device(), &layoutInfo, nullptr, &descLayout))) {
                QMessageBox::critical(main_window, "VMEmu", "Could not create descriptor set layout. Switch to another renderer. " + Vulkan_GetResultString(res));
                return;
            }
            // Sampler binding end.
            ...
            if ((res = m_devFuncs->vkCreatePipelineLayout(m_window->device(), &pipelineLayoutInfo, nullptr, &pipelineLayout)) != VK_SUCCESS) {
                QMessageBox::critical(main_window, "VMEmu", "Could not create pipeline layout. Switch to another renderer. " + Vulkan_GetResultString(res));
                return;
            }
            ...
            if ((res = m_devFuncs->vkCreateShaderModule(m_window->device(), &createInfo, nullptr, &vertModule)) != VK_SUCCESS) {
                QMessageBox::critical(main_window, "VMEmu", "Could not create vertex shader. Switch to another renderer. " + Vulkan_GetResultString(res));
                return;
            }
            ...
            if ((res = m_devFuncs->vkCreateShaderModule(m_window->device(), &createInfo, nullptr, &fragModule)) != VK_SUCCESS) {
                QMessageBox::critical(main_window, "VMEmu", "Could not create fragment shader. Switch to another renderer. " + Vulkan_GetResultString(res));
                return;
            }
            ...
            if ((res = m_devFuncs->vkCreateGraphicsPipelines(m_window->device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline)) != VK_SUCCESS) {
                ...
                QMessageBox::critical(main_window, "VMEmu", "Could not create graphics pipeline. Switch to another renderer. " + Vulkan_GetResultString(res));
                return;
            }
            m_devFuncs->vkDestroyShaderModule(m_window->device(), vertModule, nullptr);
            m_devFuncs->vkDestroyShaderModule(m_window->device(), fragModule, nullptr);
        ...
        if ((res = m_devFuncs->vkCreateSampler(m_window->device(), &samplerInfo, nullptr, &sampler)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create linear image sampler. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        }
        ...
        if ((res = m_devFuncs->vkCreateSampler(m_window->device(), &samplerInfo, nullptr, &nearestSampler)) != VK_SUCCESS) {
            QMessageBox::critical(main_window, "VMEmu", "Could not create nearest image sampler. Switch to another renderer. " + Vulkan_GetResultString(res));
            return;
        }
